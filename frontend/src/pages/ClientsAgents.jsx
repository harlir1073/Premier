...
const EMPTY_FORM = {
  type: 'agent', name: '', contact_person: '', email: '', phone: '',
  currency: 'USD', credit_terms_days: 0, commission_percent: 0, notes: ''
};

export default function ClientsAgents() {
  const { can } = useAuth();
  ...
  async function load() {
    ...
    const data = await api.get(`/clients-agents?${params}`);
    setRecords(data);
  }
  ...
  async function handleSubmit(e) {
    ...
    setError(null);
    try {
      await api.post('/clients-agents', form);
      setShowForm(false);
      setForm(EMPTY_FORM);
      load();
    } catch (err) {
    ...
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="font-display text-3xl">Clients & Agents</h1>
        </div>
        {can('clients_agents', 'create') && (
          <button
            onClick={() => setShowForm(s => !s)}
            className="bg-ink text-white rounded px-4 py-2 text-sm font-medium hover:bg-slate transition-colors"
          >
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-paper border border-slate/10 rounded-lg p-5 grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="label-eyebrow block mb-1">Credit terms (days)</label>
            <input type="number" value={form.credit_terms_days} onChange={e => setForm({ ...form, credit_terms_days: e.target.value })}
              className="w-full border border-slate/20 rounded px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="label-eyebrow block mb-1">Commission %</label>
          ...
          <div className="col-span-2">
            <button type="submit" className="bg-ink text-white rounded px-4 py-2 text-sm font-medium hover:bg-slate transition-colors">
              Save
            </button>
          </div>
        </form>
        ...
        <table className="w-full text-sm">
          <thead className="bg-cloud text-left">
            <tr>
              ...
              <th className="px-4 py-3 label-eyebrow">Commission</th>
